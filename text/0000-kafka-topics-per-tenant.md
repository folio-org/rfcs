...
# Cease Creating Kafka Topics For Each Tenant
...
- **Cost**: Kafka's use in FOLIO is still limited, more FOLIO modules will continue to employ event driven techniques that will cause an explosion of partitions. For hosting providers that self host Kafka, more brokers or broker capacity will be needed to service load generated by the high number of partitions. For hosting providers who utilize cloud managed Kafka, cloud providers typically charge per partition or set limits on partition count for a particular broker size. For example, Confluent Cloud charges $0.0015/partition/hour with a maximum of 4096 partition for one cluster. AWS charges $0.42/hour for a broker size with 4 vCPU and 16GB Memory and a soft limit of 1000 partitions per broker.

## Out of Scope
- Enabling tenant data security within a Kafka installation. Tenant data security meaning "How do we prevent a tenant's data to be processed in place of another?", "How can we prevent a tenant module from seeing data from other tenants?"
...
- **Reduce Partition Count For Each Topic**: folio-kafka-wrapper has a default of 50 partitions if a topic is created with it administrative interfaces. Partition counts can be modified further by a module. So there isn't a value that all FOLIO modules use.<p> Partition count should be tailored by a Kafka admin to suite the Kafka cluster involved. Partitions are meant to enable parallelism within a topic. A default of 50 partitions for a topic with only 2 consumers is overkill. A replication factor of 1 means that there will be 100 partitions for the topic. FOLIO has no guarantee that its configuration will be intact after initialization.<p> Reducing partition counts for each topic will produce decent impact on the problem illustrated by this document. There is still a linear relationship between the number of tenants in a FOLIO cluster and the number of partitions in a Kafka cluster. Additional functionality which require the use of more topics will mean that partition counts in the Kafka cluster will grow with the addition of new tenants and release upgrades. Partition counts will be revisited sooner rather than later when reducing only partition configuration of a topic rather than a topic's existence. The scalability of FOLIO is still bound by Kafka.<p> Partitions on a topic in Kafka can only be increased and not decreased so the topic has to be dropped and recreated which will require some application disruption event similar to what can occur if this document is taken into implementation phase.
...
```
Modifications will be made to mark the method above as deprecated and another method created to remove the wildcard which will allow the subscription pattern to look like below
>folio\.Default\.DI_COMPLETED

Safeguards will be added to ensure that a message has a tenant id as one of its headers. These include:
- Enriching messages produced by folio-kafka-wrapper with a tenant id before the message is sent.
- Discarding messages consumed that don't have a tenant id. Discarded messages can be logged in partially or with the full contents or placed in a dead queue/topic for further review.
...
Deprecated methods will allow an easier transition as processes roll off tenant specific topics to single topics.
...
- During a flower release upgrade, there should be no customer activity occurring in a FOLIO cluster. Existing topics will be deleted. Care should be taken to ensure all messages in the topic is consumed before topic deletion.
...
## Risks and Drawbacks
- FOLIO's multi-versioning scheme, involving tenants using different versions of modules, will cause functional issues. This is dependent on the breaking changes implemented in the varying versions. Multiple versions of a module will be consuming from the same topic and there are no guarantees about which module version will consume a message sourced from an incompatible version.
- Insights like "how many messages have yet to be processed for a tenant" will be harder to derive.
- Per tenant topic settings will not possible.
- [Temporary Kafka Security Solution](https://wiki.folio.org/x/YYVFAw) will no longer be possible. This solution is meant to be temporary as its name implies, a stop gap to a permanent solution. Creating topics for each tenant has been implemented and lightly enforced via the [folio-kafka-wrapper](https://github.com/folio-org/folio-kafka-wrapper) library. FOLIO is not able to enforce tenant security and isolation within its boundaries: the other two parts are not defined within FOLIO code, it is up to the hosting provider to implement. 
...
### Finishing Temporary Kafka Security Solution
...
### Supporting Module Multi-Versioning Via Topic Versioning
...
### Supporting Module Multi-Versioning Via Message Versioning
