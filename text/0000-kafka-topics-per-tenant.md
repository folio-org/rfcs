...
## Out of Scope
- Enabling tenant data security within a Kafka installation. Tenant data security meaning "How do we prevent a tenant's data from being processed in place of another?", "How can we prevent a tenant module from seeing data from other tenants?"
...
- FOLIO modules that do not use folio-kafka-wrapper to interact with Kafka.
...
[folio-kafka-wrapper](https://github.com/folio-org/folio-kafka-wrapper) will check for the existence of an environment variable called `KAFKA_PRODUCER_TENANT_COLLECTION`. With this variable unset, the module will produce to topics for each tenant. if the variable is set to a value like `ALL`, the module will produce to one topic belonging to the "ALL tenant" e.g. `folio.Default.ALL.DI_COMPLETED`.<p>
